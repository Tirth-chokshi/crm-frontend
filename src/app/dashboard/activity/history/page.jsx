...
const ActivityHistory = () => {
  ...
  useEffect(() => {
    const fetchActivities = async () => {
      try {
        const response = await fetch('http://localhost:8000/activities/history');
  ...
  const responseTypes = ['all', ...new Set(activities.map(a => a.customer_response))];
  ...
  const filteredActivities = activities.filter(activity => {
    ...
    const matchesResponse = filterResponse === 'all' || activity.customer_response === filterResponse;
              ...
              {filteredActivities.length === 0 ? (
                ...
                filteredActivities.map((activity) => (
                  <TableRow key={activity.customer_activity_id}>
                    ...
                    <TableCell>
                      <Badge className={getResponseBadgeColor(activity.customer_response)}>
                        {activity.customer_response}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <Badge 
                        variant={activity.case_resolved === "Resolved" ? "success" : "secondary"}
                      >
